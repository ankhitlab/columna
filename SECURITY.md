...
- **Paths and URLs from outside the program.** Readers accept a bare string and guess whether it is content, a path or a URL; that guess is convenience, not a boundary. Server-side code must tag the source (`{ text }` / `{ path }` / `{ url }` or `mode`) and set a policy — `allowedHosts`, `denyPrivateHosts`, `allowedDirs`, `maxBytes`, `timeoutMs`, `signal` — per call or once via `setIoPolicy()` (a floor that per-call options cannot widen). Without a policy the library reads whatever the process can reach; that is the documented default, not a vulnerability, and `denyPrivateHosts` does not resolve DNS (pass a pinning `fetch` if rebinding is in scope).
...
- **Studio.** The REPL executes user-supplied JavaScript in the page's own realm, with the page's privileges (opened folder handles, origin storage, network) — it is a devtools console with a nicer UI, not a sandbox, and the dev server binds to loopback for that reason. Opening someone else's project or notebook in Studio is running their code. A Worker / iframe would not be a trust boundary (same origin, same handles); proper isolation would be a separate design. See `packages/studio/README.md`.
