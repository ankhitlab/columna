...
## Глубина валидации — что именно подтверждено, а что нет

Три слоя тестов на каждую функцию (эталон → свойства → Монте-Карло) подтверждают совпадение на «удобных» выборках.
Отдельный набор [tests/adversarial.test.ts](../packages/advanced/tests/adversarial.test.ts) (эталоны —
`tests/refs/adversarial_ref.py`, scipy 1.14) проверяет то, где оценщики обычно ломаются, и **каждый случай сверен со
специализированным методом эталонной библиотеки**, а не с одноимённой функцией:

| Класс | Случаи | Эталон |
|---|---|---|
| Вырожденные / плохо обусловленные матрицы | точная коллинеарность (x₃ = x₁ + x₂ → алиас, fitted/SSE единственны), cond ≈ 10⁷ (полиномиальные столбцы), насыщенная подгонка p = n (отказ), константный предиктор (алиас с константой), константный отклик (R² = NaN) | `numpy.linalg.lstsq`, `matrix_rank`, `cond` |
| Пропуски | t-тест с null в обеих выборках, Пирсон с попарным удалением | scipy на очищенных данных |
| Совпадающие ранги | Манн–Уитни с массовыми связками (поправка + непрерывность), Уилкоксон с нулями и связками (`zero_method='wilcox'`), Краскел–Уоллис, Спирмен / Кендалл τ-b | `mannwhitneyu(method='asymptotic')`, `wilcoxon`, `kruskal`, `spearmanr`, `kendalltau` |
| Константные выборки | t на константе (±∞ / NaN по соглашению scipy, без исключения), ANOVA и Левене с константной группой, корреляция с константой → NaN | `ttest_1samp`, `f_oneway`, `levene` |
| Крайние вероятности | normal/t/χ²/F/binom/Poisson/beta/gamma: хвосты до 10⁻³⁰⁰ и 1 − 10⁻¹⁶, n = 10 000, λ = 10⁵, формы 0.05 и 10⁻³ | scipy.stats `cdf/sf/ppf/pmf` |
| Односторонние альтернативы | t ×4 (с ДИ), точные Манн–Уитни и Уилкоксон, Пирсон (с ДИ), точная биномиальная доля при k = 0, n, 3, 19 (Клоппер–Пирсон), знаковый, χ² дисперсии, точный пуассоновский | `alternative='less'/'greater'` во всех, `binomtest`, `proportion_ci(method='exact')` |
| Таблицы 2×2 с пустыми клетками | [[0,10],[10,0]], [[100,0],[0,100]], [[1,0],[0,1]] … | `fisher_exact` (3 альтернативы), `chi2_contingency` с поправкой Йетса и без |

Этот набор нашёл и закрыл два численных дефекта, которых happy-path не видел: усечение ряда неполной гамма-функции
на 1000 членах (Poisson sf при λ = 10⁵ — ошибка 2·10⁻⁷; теперь бюджет итераций растёт как √a) и квантиль бета-распределения
с малыми параметрами (бисекция по x упиралась в 10⁻¹⁶ и возвращала это вместо 10⁻¹¹⁵; теперь инверсия в log-пространстве).
Два расхождения оказались соглашениями, а не ошибками, и задокументированы в тесте: Уилкоксон возвращает W⁺ (Minitab), scipy —
min(W⁺, W⁻); двусторонний точный p для Пуассона — «minlike» (как `binomtest` и Minitab), а не удвоенный хвост.

Что **не** подтверждено этим набором: ARIMA/ETS/STL против statsmodels (не установлен; сверка идёт с собственными реализациями
формул и с numpy там, где они закрытые), MVP-методы из `minitab-coverage.md` со статусом «MVP» — они сверены только на
эталонных примерах, и поведение на вырожденных входах для них проверено лишь на «не падать» (`robustness.test.ts`).
